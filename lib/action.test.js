...
describe('pr-lint-action', () => {
  ...
  describe('checking status', () => {
    ...
    it('skips validation for dependabot PRs', async () => {
      const dependabotPR = {
        ...bad_title_and_branch,
        user: { login: 'dependabot[bot]' }
      };
      context.payload = pullRequestOpenedFixture(dependabotPR);

      await action(context, api);
      expect(console.log).toHaveBeenCalledWith('Skipping Shortcut validation for Dependabot PR');
...
function pullRequestOpenedFixture({ title, ref_name, body, user }) {
  return {
    pull_request: {
      number: 1,
      title,
